...
export const AddTransactionModal = ({ open, onClose, onSave, finance, initial }: AddTransactionModalProps): JSX.Element | null => {
  ...
  return (
    ...
    >
      ...
      >
        <h3 id="transaction-modal-title" className="text-lg font-semibold">
          {initial ? 'Editar transacción' : 'Nueva transacción'}
        </h3>

        <div className="grid grid-cols-2 gap-2" role="group" aria-label="Tipo de transacción">
          <button
            type="button"
            onClick={() => handleTypeChange('income')}
            aria-pressed={payload.type === 'income'}
          ...
          <button
            ...
            aria-pressed={payload.type === 'expense'}
        ...
        <div className="rounded-[12px] bg-[var(--bg-surface)] px-4 py-3 text-center">
          <label htmlFor="transaction-amount" className="mb-1 block text-[10px] uppercase tracking-[2px] text-[var(--text-muted)]">Monto</label>
          <div className="flex items-end justify-center gap-2 border-b-2 border-white/10 pb-1">
            <span className="text-xl text-[var(--text-muted)]" aria-hidden="true">RD$</span>
            <input
              ...
              min="0"
              placeholder="0.00"
              aria-label="Monto en pesos dominicanos"
        ...
        <label className="block text-sm">
          ...
          <input
            ...
            className="w-full rounded-[10px] bg-[var(--bg-surface)] px-3 py-2 text-[var(--text-primary)]"
          />
        </label>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <label className="text-sm">
            ...
            <input value={payload.note} onChange={(e) => setPayload((p) => ({ ...p, note: e.target.value }))} placeholder="Opcional" className="w-full rounded-[10px] bg-[var(--bg-surface)] px-3 py-2" />
          </label>
        </div>

        {error && (
          <p className="rounded-[10px] bg-[rgba(255,69,96,0.1)] px-3 py-2 text-xs text-[var(--expense)]" role="alert">
            {error}
          </p>
        )}

        <div className="flex justify-end gap-2">
          <button type="button" onClick={onClose} className="rounded-[10px] border border-[var(--border)] px-4 py-2 text-sm text-[var(--text-secondary)] transition hover:text-[var(--text-primary)]">
            Cancelar
          </button>
